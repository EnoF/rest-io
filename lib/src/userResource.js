...
var UserResource = (function (_super) {
    ...
    UserResource.prototype.update = function (req, res) {
        ...
        this.isAuthorized(req, this.roles.update)
            .then(function () { return _super.prototype.update.call(_this, req, res); }, function (err) {
            if (err.message === 'unauthorized' && _this.isSelf(req)) {
                return _this.model.findByIdAndUpdate(req.params.userId, req.body);
            }
            throw err;
        })
            .then(function (user) { return res.send(user); }, function (err) { return _this.sendUnauthorized(err, res); });
    };
